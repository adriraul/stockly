...
class DatabaseService {
  ...
  async init(): Promise<void> {
    try {
      console.log('OPEN database: StocklyDB_v3.db');
      this.db = await SQLite.openDatabaseAsync('StocklyDB_v3.db');
      console.log(
        'SQLite.open({"name":"StocklyDB_v3.db","location":"default","dblocation":"nosync"})',
      );
